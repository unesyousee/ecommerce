                        ...
                        @foreach ($choices as $choice)
                            <div class="product-layout product-list col-xs-12">
                                <div class="product-thumb">
                                    ...
                                    @foreach($choice->pic as $pic)
                                        @if($pic->main==1)
                                    <div class="image"><a href="{{ URL::to('/show/'.$choice->id) }}"><img src="http://idpz.ir/assets/images/goods/{{$choice->pic->first()->pic}}" alt="{{$choice->name}}" title="{{$choice->name}}" class="img-responsive" /></a></div>
                                        @endif
                                    @endforeach

                                    <div>
                                        <div class="caption">
                                            <h4><a href="{{ URL::to('/show/'.$choice->id) }}"> {{$choice->name}} </a></h4>
                                            {{--<p class="description"> آخرین دستاورد های روز دنیا--}}

                                                {{--شامل تمام اطلاعاتی که به آن نیاز خواهید داشت ...</p>--}}
                                            <p class="price"> <span class="price-new">{{$choice->price}}</span>
                                                @if($choice->old_rice==null )
                                                @elseif($choice->old_price == price)
                                                @else
                                                    <span class="price-old">{{$choice->old_price}}</span>
                                                @endif
                                            </p>
                                        </div>
                                        <div class="button-group">
                                            <p class="btn-primary" type="button"><a class="btn-primary"  href="{{ URL::to('/add-to-cart/'.$choice->id) }}">افزودن به سبد</a> </p>                                                    {{--<div class="add-to-links">--}}
